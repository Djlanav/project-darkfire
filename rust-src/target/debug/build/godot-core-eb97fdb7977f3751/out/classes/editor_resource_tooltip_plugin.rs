...
pub(super) mod re_export {
    ...
    impl EditorResourceTooltipPlugin {
        pub fn request_thumbnail(&self, path: impl AsArg < GString >, control: impl AsObjectArg < crate::classes::TextureRect >,) {
            type CallSig < 'a0, > = ((), CowArg < 'a0, GString >, ObjectArg < crate::classes::TextureRect >);
            let args = (path.into_arg(), control.as_object_arg(),);
            unsafe {
                let method_bind = sys::class_editor_api() . fptr_by_index(259usize);
                < CallSig as PtrcallSignatureTuple > ::out_class_ptrcall(method_bind, "EditorResourceTooltipPlugin", "request_thumbnail", self.object_ptr, self.__checked_id(), args,)
            }
        }
    ...
    macro_rules !unsafe_inherits_transitive_EditorResourceTooltipPlugin {
        ($Class: ident) => {
