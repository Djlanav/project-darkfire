...
pub(super) mod re_export {
    ...
    impl JsonRpc {
        pub fn set_scope(&mut self, scope: impl AsArg < GString >, target: impl AsObjectArg < crate::classes::Object >,) {
            type CallSig < 'a0, > = ((), CowArg < 'a0, GString >, ObjectArg < crate::classes::Object >);
            let args = (scope.into_arg(), target.as_object_arg(),);
            unsafe {
                let method_bind = sys::class_scene_api() . fptr_by_index(4540usize);
                < CallSig as PtrcallSignatureTuple > ::out_class_ptrcall(method_bind, "JsonRpc", "set_scope", self.object_ptr, self.__checked_id(), args,)
            }
        }
        pub(crate) fn process_action_full(&mut self, action: RefArg < Variant >, recurse: bool,) -> Variant {
            type CallSig < 'a0, > = (Variant, RefArg < 'a0, Variant >, bool);
            let args = (action, recurse,);
            unsafe {
                let method_bind = sys::class_scene_api() . fptr_by_index(4541usize);
                < CallSig as PtrcallSignatureTuple > ::out_class_ptrcall(method_bind, "JsonRpc", "process_action", self.object_ptr, self.__checked_id(), args,)
            }
        }
        ...
        pub fn process_string(&mut self, action: impl AsArg < GString >,) -> GString {
            ...
            unsafe {
                ...
                < CallSig as PtrcallSignatureTuple > ::out_class_ptrcall(method_bind, "JsonRpc", "process_string", self.object_ptr, self.__checked_id(), args,)
            }
        }
        pub fn make_request(&mut self, method: impl AsArg < GString >, params: &Variant, id: &Variant,) -> Dictionary {
            type CallSig < 'a0, 'a1, 'a2, > = (Dictionary, CowArg < 'a0, GString >, RefArg < 'a1, Variant >, RefArg < 'a2, Variant >);
            let args = (method.into_arg(), RefArg::new(params), RefArg::new(id),);
            unsafe {
                let method_bind = sys::class_scene_api() . fptr_by_index(4543usize);
                < CallSig as PtrcallSignatureTuple > ::out_class_ptrcall(method_bind, "JsonRpc", "make_request", self.object_ptr, self.__checked_id(), args,)
            }
        }
        pub fn make_response(&mut self, result: &Variant, id: &Variant,) -> Dictionary {
            type CallSig < 'a0, 'a1, > = (Dictionary, RefArg < 'a0, Variant >, RefArg < 'a1, Variant >);
            let args = (RefArg::new(result), RefArg::new(id),);
            unsafe {
                let method_bind = sys::class_scene_api() . fptr_by_index(4544usize);
                < CallSig as PtrcallSignatureTuple > ::out_class_ptrcall(method_bind, "JsonRpc", "make_response", self.object_ptr, self.__checked_id(), args,)
            }
        }
        pub fn make_notification(&mut self, method: impl AsArg < GString >, params: &Variant,) -> Dictionary {
            type CallSig < 'a0, 'a1, > = (Dictionary, CowArg < 'a0, GString >, RefArg < 'a1, Variant >);
            let args = (method.into_arg(), RefArg::new(params),);
            unsafe {
                let method_bind = sys::class_scene_api() . fptr_by_index(4545usize);
                < CallSig as PtrcallSignatureTuple > ::out_class_ptrcall(method_bind, "JsonRpc", "make_notification", self.object_ptr, self.__checked_id(), args,)
            }
        }
        pub(crate) fn make_response_error_full(&self, code: i32, message: CowArg < GString >, id: RefArg < Variant >,) -> Dictionary {
            type CallSig < 'a0, 'a1, > = (Dictionary, i32, CowArg < 'a0, GString >, RefArg < 'a1, Variant >);
            let args = (code, message, id,);
            unsafe {
                let method_bind = sys::class_scene_api() . fptr_by_index(4546usize);
                < CallSig as PtrcallSignatureTuple > ::out_class_ptrcall(method_bind, "JsonRpc", "make_response_error", self.object_ptr, self.__checked_id(), args,)
            }
        }
    ...
    macro_rules !unsafe_inherits_transitive_JsonRpc {
        ($Class: ident) => {
